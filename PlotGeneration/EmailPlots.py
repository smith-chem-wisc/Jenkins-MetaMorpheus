...
msgRoot["Subject"] = "{reportType} Jenkins Build Results {date}".format(
...
file_path = os.path.join(
    os.path.dirname(folder), "MetaMorpheus_MasterBranch\\MetaMorpheus\\GUI\\GUI.csproj"
)
mzLibVersion = extract_version(file_path)

response = requests.get(
    "https://api.github.com/repos/smith-chem-wisc/MetaMorpheus/releases/latest"
)
mmVersion = response.json()["tag_name"]
response = requests.get(
    "https://api.github.com/repos/smith-chem-wisc/MetaMorpheus/commits/master"
)
...
html_content = """
<b>{reportType} Jenkins Report</b><br>
...
<p><img src="cid:mastergrid" style="width:100%; max-width:1600px; height:auto;" /></p>
""".format(
    reportType=reportType,
    mmVersion=mmVersion,
    latestCommitMessage=latestCommitMessage,
    latestCommitHash=latestCommitHash,
    mzLibVersion=mzLibVersion,
)

msgText = MIMEText(html_content, "html")
msgAlternative.attach(msgText)

# Attach the canonical report image to the email
image_path = os.path.join(reportdirectory, "MasterGrid.png")
with open(image_path, "rb") as fp:
    msgImage = MIMEImage(fp.read())
msgImage.add_header("Content-ID", "<mastergrid>")
